...
def source_matches_original(data: bytes, expected_sha256: str) -> bool:
    """Accept original bytes or Git's LF-to-CRLF checkout conversion only.

    Keep existing raw hashes valid. For LF originals, tolerate CRLF line
    endings without changing spaces, tabs, encoding, or the final newline.
    The preservation manifest is never regenerated by this check.
    """
    if hashlib.sha256(data).hexdigest() == expected_sha256:
        return True
    lf_data = data.replace(b'\r\n', b'\n')
    return hashlib.sha256(lf_data).hexdigest() == expected_sha256
...
def check_source():
    ...
    for item in json.loads((ROOT/'tests/ui_preservation_manifest.json').read_text()):
        ...
        if not item['modified']:
            assert source_matches_original(f.read_bytes(), item['original_sha256']), (
                f"{item['path']}: source SHA256 mismatch "
                "even after CRLF-to-LF normalization"
            )
