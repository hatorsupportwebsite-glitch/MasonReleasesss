...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float square_sz = 14 * GetStyle().Scale;
    ...
    static std::unordered_map<ImGuiID, picker_state> anim;
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32, hue_color32, col_white, 6);
        draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0, col_black, col_black, 6);
        ...
        // // Render Hue Bar
        // for (int i = 0; i < 6; ++i)
        //     draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1], 5, i == 0 ? ImDrawFlags_RoundCornersTop : i == 5 ? ImDrawFlags_RoundCornersBottom : ImDrawFlags_RoundCornersNone);
    }

    // float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
    // bar0_line_y = ImClamp(bar0_line_y, picker_pos.y + 3.f * style.Scale, picker_pos.y + (sv_picker_size - 13 * style.Scale));
    //
    // state.hue_bar = ImLerp(state.hue_bar, bar0_line_y + 5 * style.Scale, g.IO.DeltaTime * 24.f);
    // draw_list->AddCircleFilled(ImVec2(bar0_pos_x + 7.f * style.Scale, state.hue_bar), 4.5f * style.Scale, col_black, 100.f);

    float sv_cursor_rad = value_changed_sv ? 10.0f * style.Scale : 6.0f * style.Scale;
    ...
    state.circle = ImLerp(state.circle, value_changed_sv ? 6.0f * style.Scale : 4.0f * style.Scale, g.IO.DeltaTime * 24.f);

    draw_list->AddShadowCircle(state.circle_move, state.circle, ImColor(0.f, 0.f, 0.f, style.Alpha), 8 * style.Scale, { 0, 0 }, 0);

    draw_list->AddCircle(state.circle_move, state.circle, col_white, sv_cursor_segments, 3.f * style.Scale);
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min, bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f), 4);
        draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, 4);

        float bar1_line_y = IM_ROUND(picker_pos.y + (1.0f - alpha) * sv_picker_size);
        bar1_line_y = ImClamp(bar1_line_y, picker_pos.y + 3.f * style.Scale, picker_pos.y + (sv_picker_size - 13 * style.Scale));

        state.alpha_bar = ImLerp(state.alpha_bar, bar1_line_y + 5 * style.Scale, g.IO.DeltaTime * 24.f);
        draw_list->AddCircleFilled(ImVec2(bar1_pos_x + 7.f * style.Scale, state.alpha_bar), 4.5f * style.Scale, col_black, 100.f);
    }
